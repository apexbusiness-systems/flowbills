                ...
                <div className="bg-muted/50 rounded-lg p-3">
                  ...
                  <div className="text-sm text-muted-foreground">
                    {rule.rule_type === "amount_range" && (
                      <div>
                        {rule.rule_config.min_amount && (
                          <span>Min: ${rule.rule_config.min_amount} </span>
                        )}
                        {rule.rule_config.max_amount && (
                          <span>Max: ${rule.rule_config.max_amount}</span>
                        )}
                      </div>
                    )}
                    {rule.rule_type === "required_fields" && (
                      <div>Required: {(rule.rule_config.required_fields || []).join(", ")}</div>
                    )}
                    {rule.rule_type === "date_validation" && (
                      <div>
                        {rule.rule_config.check_future_dates && <span>No future dates </span>}
                        {rule.rule_config.max_days_old && (
                          <span>Max age: {rule.rule_config.max_days_old} days</span>
                        )}
                      </div>
                    )}
                    ...
                    {rule.rule_type === "duplicate_check" && (
                      <div>
                        Check fields:{" "}
                        {(rule.rule_config.check_fields || ["invoice_number"]).join(", ")}
                      </div>
                    )}
                    {rule.rule_type === "format_validation" && (
                      <div>
                        {rule.rule_config.invoice_number_pattern && (
                          <span>Invoice format check </span>
                        )}
