...
class MobileResponsivenessAuditor {
  ...
  private checkViewportMeta() {
    ...
    if (!viewport) {
      this.addIssue('critical', 'layout', 'Missing viewport meta tag', 'head', 
        'Add <meta name="viewport" content="width=device-width, initial-scale=1">');
    } else {
      const content = viewport.getAttribute('content') || '';
      if (!content.includes('width=device-width')) {
        this.addIssue('warning', 'layout', 'Viewport not set to device width', 'meta[name="viewport"]',
  ...
  private checkTouchTargets() {
    ...
    clickableElements.forEach((element) => {
      ...
      if (rect.width < minSize || rect.height < minSize) {
        this.addIssue('warning', 'interaction', 
  ...
  private checkContentOverflow() {
    ...
    elements.forEach((element) => {
      ...
      if (style.overflow === 'visible' && element.scrollWidth > element.clientWidth) {
        this.addIssue('warning', 'layout', 'Content overflow detected',
          this.getElementSelector(element as HTMLElement),
  ...
  private checkResponsiveImages() {
    ...
    images.forEach((img) => {
      ...
      if (!style.maxWidth || style.maxWidth === 'none') {
        this.addIssue('info', 'content', 'Image may not be responsive',
          this.getElementSelector(img),
          'Add max-width: 100% or use responsive image classes');
      }
      
      if (!img.getAttribute('alt')) {
        this.addIssue('warning', 'content', 'Missing alt text',
          this.getElementSelector(img),
  ...
  private checkMobileNavigation() {
    ...
    const mobileToggle = nav.querySelector('[data-mobile-toggle], [aria-expanded], .hamburger, .menu-toggle');
    if (!mobileToggle && window.innerWidth < 768) {
      this.addIssue('critical', 'interaction', 'No mobile navigation toggle found',
        'nav', 'Add hamburger menu or mobile navigation toggle');
    }
    ...
    navLinks.forEach((link) => {
      ...
      if (rect.height < 44) {
        this.addIssue('warning', 'interaction', 'Navigation link too small for touch',
  ...
  private checkMobileForms() {
    ...
    forms.forEach((form) => {
      ...
      inputs.forEach((input) => {
        ...
        // Check input size
        if (rect.height < 44) {
          this.addIssue('warning', 'interaction', 'Form input too small for touch',
        ...
        if (htmlInput.type === 'text') {
          ...
          if (name.includes('email')) {
            this.addIssue('info', 'interaction', 'Consider using email input type',
              this.getElementSelector(htmlInput),
              'Use type="email" for better mobile keyboard');
          }
          if (name.includes('phone') || name.includes('tel')) {
            this.addIssue('info', 'interaction', 'Consider using tel input type',
              this.getElementSelector(htmlInput),
  ...
  private async checkMobilePerformance() {
    if ('connection' in navigator) {
      ...
      if (connection && connection.effectiveType) {
        ...
        if (slowConnections.includes(connection.effectiveType)) {
          this.addIssue('info', 'performance', 'Slow network connection detected',
            'global', 'Consider optimizing images and reducing bundle size');
        }
      }
    }
    ...
    images.forEach((img) => {
      if (img.naturalWidth > 1200 || img.naturalHeight > 1200) {
        this.addIssue('warning', 'performance', 'Large image detected',
          this.getElementSelector(img),
  ...
  private addIssue(type: MobileIssue['type'], category: MobileIssue['category'], 
