...
export function validateFacturX(xmlContent: string): ValidationResult {
  ...
  if (!xmlContent.includes('CrossIndustryDocument') && !xmlContent.includes('CrossIndustryInvoice')) {
    errors.push('Missing root element: CrossIndustryDocument/CrossIndustryInvoice (required for Factur-X)');
  }
  ...
  if (!xmlContent.includes('urn:factur-x.eu:1p0') && !xmlContent.includes('urn:cen.eu:en16931:2017')) {
...
export function buildBIS30(invoiceXml: string, metadata: {
  ...
  const docTypeId = metadata.document_type_id || 'urn:oasis:names:specification:ubl:schema:xsd:Invoice-2';
