...
export const useFieldTickets = () => {
  ...
  const fetchFieldTickets = useCallback(async (filters?: {
    verified?: boolean;
    invoice_id?: string;
    afe_id?: string;
    search?: string;
    ...
    try {
      ...
      if (filters?.search) {
        query = query.or(`ticket_number.ilike.%${filters.search}%,vendor_name.ilike.%${filters.search}%,location.ilike.%${filters.search}%`);
      }
  ...
  const createFieldTicket = useCallback(async (ticketData: {
    ticket_number: string;
    vendor_name: string;
    service_date: string;
    amount: number;
  ...
  const updateFieldTicket = useCallback(async (
    ticketId: string,
  ...
  const verifyFieldTicket = useCallback(async (ticketId: string, notes?: string) => {
    if (!user) return false;
  ...
  const linkToInvoice = useCallback(async (ticketId: string, invoiceId: string) => {
    return updateFieldTicket(ticketId, { invoice_id: invoiceId });
  }, [updateFieldTicket]);

  const deleteFieldTicket = useCallback(async (ticketId: string) => {
    if (!user) return false;
  ...
  const getFieldTicketById = useCallback((id: string) => {
