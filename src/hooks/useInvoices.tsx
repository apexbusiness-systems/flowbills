...
export const useInvoices = () => {
  ...
  const fetchPage = useCallback(
    async (cursor?: { created_at: string; id: string }): Promise<Page> => {
      ...
      const result = await queryOptimizer.supabaseQuery(
        ...
        async (client) => {
          ...
          if (cursor) {
            q.lt("created_at", cursor.created_at).or(
  ...
  const createInvoice = async (
    invoiceData: Omit<Invoice, "id" | "created_at" | "updated_at" | "user_id" | "vendor_name">
  ) => {
    if (!user) return null;
  ...
  const updateInvoice = async (id: string, updates: Partial<Invoice>) => {
    ...
    try {
      ...
      const mappedInvoice = { ...data, vendor_name: "Unknown Vendor" } as Invoice;
      setInvoices((prev) => prev.map((invoice) => (invoice.id === id ? mappedInvoice : invoice)));
