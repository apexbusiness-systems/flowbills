...
export const useUWIs = () => {
  ...
  const fetchUWIs = useCallback(
    async (filters?: { status?: string; province?: string; search?: string }) => {
      if (!user) return;
      ...
      try {
        ...
        if (filters?.search) {
          query = query.or(
            `uwi.ilike.%${filters.search}%,well_name.ilike.%${filters.search}%,operator.ilike.%${filters.search}%`
          );
        }
  ...
  const getAssociatedData = useCallback(
    async (uwiId: string) => {
      ...
      try {
        const [invoiceExtractions, fieldTickets] = await Promise.all([
          supabase
            ...
            .eq("uwi_id", uwiId)
            .eq("user_id", user.id),
          supabase.from("field_tickets").select("*").eq("uwi_id", uwiId).eq("user_id", user.id),
        ]);
