...
export const useAFEs = () => {
  ...
  const fetchAFEs = useCallback(async (filters?: {
    status?: string;
    search?: string;
  }) => {
    if (!user) return;
    ...
    try {
      ...
      if (filters?.search) {
        query = query.or(`afe_number.ilike.%${filters.search}%,description.ilike.%${filters.search}%,well_name.ilike.%${filters.search}%`);
      }
  ...
  const createAFE = useCallback(async (afeData: {
    afe_number: string;
    description?: string;
    budget_amount: number;
    well_name?: string;
  ...
  const updateAFE = useCallback(async (
    afeId: string,
  ...
  const deleteAFE = useCallback(async (afeId: string) => {
    if (!user) return false;
  ...
  const getAFEById = useCallback((id: string) => {
